...
| `gen_readme.py` | Rewrites the README's generated blocks (the Apps table with one row, download and Obtainium badge per app and channel, and the `[obt-*]` link targets) from `_site/api/*.json`. | the `readme` job in `site.yml` |
