...
| `flow-test.mjs` | Drives the published page like a visitor (renders `app.js` under jsdom against the live site; `SITE_DIR=site/static` tests local page files against the live API before they ship) and asserts every feature is wired: cards in the right channel, no stray `null`/`undefined` text, every APK link (latest, history, Obtainium, README and every Releases asset) downloads at the listed size and carries its app's package, download links, All-versions history, catalog tab, search, API tab; every Obtainium config replayed the way Obtainium reads it, each id checked against the package the APK really declares, the bulk-add page, and the README's generated rows against the live API and `config.toml`. | manually |
